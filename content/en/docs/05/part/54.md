...
Since we had a brief look at the configuration we are now going to configure the `outboxevent` table. We will also create an event history table `consumedevent` to ignore events which have already been processed (idempotency).
