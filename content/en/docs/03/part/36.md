...
The LRA MicroProfile is built for microservices with RESTful communication. Using LRA each microservice participating in a LRA will have
...
Further the library uses the base url extracted form the request-uri to register itself at the LRA Coordinator. Our
...
Add the following environment variables to the `order` and `stock` container to fix issues with bridged docker networking. For details, see the Section LRA Coordinator above.
```yaml
  order:
    # ... omitted
    environment:
