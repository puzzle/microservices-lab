...
This is the final structure of our `order` microservice at the end of the tasks in this section. You will be guided the implementation of these:
```
.
├── pom.xml
└── src
...
{{% alert title="PanacheEntity" color="warning" %}} We are using the Quarkus Hibernate ORM with Panache extension. The
classical approach with the EntityManager would work as well and might be required for complex use cases. However, in
our simple implementation we will make use of Panache. For more information about Panache have a look at: [Simplified Hibernate ORM with Panache](https://quarkus.io/guides/hibernate-orm-panache) {{% /alert %}}
