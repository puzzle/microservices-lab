...
Therefore, we will mostly only work on the `order` microservice. If you have to make changes to the `stock` microservice
...
Since our dependencies and configuration is complete, we can start to building the database schema. We will use the
...
{{% alert title="Naming" color="warning" %}} Please be precise with the naming of tables and fields, or you might run
into trouble in later steps. {{% /alert %}}

Create the `shoporder` table in the `changeLog.xml` according to the details below. This table will store our orders.
```text
admin=# \d shoporder;
                     Table "shoporder"
...
Create the `articleorder` table in the `changeLog.xml` according to the details below. This table reflects the effective
...
Create the mapping table in the `changeLog.xml` according to the details below. This table is used to build the mapping
...
Finally, create the hibernate sequence in the `changeLog.xml` according to the details below.
```text
...
The next step in building our application is to define our Domain Entities. For our `order` microservice we will need
the following entities:

* `ShopOrder`: The entitiy representing an order
* `ArticleOrder`: The entity representing an ordered Article with its amount.

Further we will create an enum `ShopOrderStatus` representing the status of the order.

We will trigger a new Order using a RESTful API call to our `order` microservice. For this we will use DTOs instead of
entities. Therefore, we need to define the following DTOs:

* `ShopOrderDTO`: A Data Transfer Object to specify which articles have been ordered with this order.
* `ArticleOrderDTO`: A Data transfer Object to specify the article id and the amount of items ordered.


### Task {{% param sectionnumber %}}.3 - Implement entities and enum

As an overview here is our domain model for the `order` microservice.
...
* Implement the entities `ShopOrder`, `ArticleOrder` and the enum `ShopOrderStatus` as defined in the image above.
  * Do not forget to annotate the entities with `@Entity` and extending from `PanacheEntity` which provides us an `id`.
  * For the `ShopOrder` entity: Annotate the field `articleOrders` with `@OneToMany` and `status` with `@Enumerated(EnumType.STRING)`
...
public class ShopOrder extends PanacheEntity {
    ...
    // for simplicity getters ans setters are omitted.
}
```
{{% /details %}}
...
The effective processing of a new order will be delegated to the `ShopOrderService`.


### Task {{% param sectionnumber %}}.5 - Create service for order creation

* Create the `ShopOrderService` in `src/main/java/ch/puzzle/mm/rest/order/control`
  * Use the scope annotation `@ApplicationScope` for the ShopOrderService

Use the following method body to create a new `ShopOrder`, set status and given articles and save the order in the database.
...
Your ShopOrderService should look like this:
```java
@ApplicationScoped
...
* **Microprofile Fault-Tolerance**[^3]: Provides us annotations such as `@Timeout`, `@Fallback`, `@Retry` or `@CircuitBreaker`


### Task {{% param sectionnumber %}}.5 - Rest Client and Fault Tolerance dependencies
...
`io.quarkus`             | `quarkus-smallrye-fault-tolerance`     | Implementation of the MicroProfile Fault Tolerance specification | [Fault Tolerance](https://quarkus.io/guides/microprofile-fault-tolerance)
...
### Task {{% param sectionnumber %}}.6 - Define the Rest Client
...
* Create the interface `ArticleStockService` in `src/main/java/ch/puzzle/mm/rest/stock/boundary`
  * Annotate the RestClient with `@RegisterRestClient` with the configKey `article-stock-api`
...
To make the rest client work we have to add configuration to our `src/main/resources/application.properties`. Add the
...
Our Microservice will provide a RESTful API to create and list existing orders. For this we need to create a Json Rest Service.

For further details have a look at the Quarkus Guide [Writing Json Rest Services](https://quarkus.io/guides/rest-json).

The implementation is pretty straightforward. We have to define a ShopOrderResource endpoint and use the JAX-RS annotations.
...
### Task {{% param sectionnumber %}}.5 - JSON RESTful Resource

Build the `ShopOrderResource` in `src/main/java/ch/puzzle/mm/rest/order/boundary` according to the simple implementation above.

* Inject the `ShopOrderService` to process the new order
* Inject the `ArticleStockService` to call our `stock` microservice
