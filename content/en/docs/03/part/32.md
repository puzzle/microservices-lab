...
{{% alert title="Environment Setup" color="warning" %}} With completing this task the environment is configured. However,
in the next few steps we will build the order microservice and its container. This container is needed to start up the
environment. {{% /alert %}}
