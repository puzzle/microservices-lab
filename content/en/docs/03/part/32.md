...
* Change occurrence of `monolith` to `rest` this should apply to the `hostname`, `container_name` and `networks` specification.

If you are done with the microservice config we referenced two database containers.

* Create the database configs `order-db` and `stock-db` with help of the existing `application-db`
  * Use `5432` as HostPort for `order-db` and `5433` as HostPort for `stock-db`
  * Name their volumes `order-db-data` and `stock-db-data`
* Edit the `volumes` block to contain the referenced volumes
